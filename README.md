...
* Use the [docker](https://www.docker.com) container service with docker-compose to install postgres-hasura.
...
* ```hasura migrate create baseline --from-server``` - Exports a snapshot of the current database schema named ```baseline``` to the ```migrations``` directory. Use ```--schema myschema1,myschema2``` to export a specific schema.
